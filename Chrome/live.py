...
if ask_port == True:
    ...
    c_port = "/dev/tty.usbserial-DM02JGYO" #port for spiker box

driver = webdriver.Chrome('/Users/hwyz/programming/chromedriver')
driver.get("https://youtu.be/WEJgPppk_kk") ##video link
print("website loaded")
video = driver.find_element_by_id('movie_player')

def take_action(my_input):
    if my_input == "space":
...
try:
    ...
    raise Exception('Could not open port. For Mac, use terminal command ls /dev/tty.* to check for avaliable ports.')
...
total_time = 6000.0; # time in seconds [[1 s = 20000 buffer size]]
...
for k in range(0,int(N_loops)):
    ...
    if(len(data_cache) == 4):
        ...
        if(k == step_count + 1):
            print("combined_data:")
            print(combined_data)
            print(tt_new(combined_data,video))
